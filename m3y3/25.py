...
def change_state(e, current_state, background_sound):
    if e.type == pg.KEYDOWN:
        if e.key == pg.K_SPACE:
            if current_state == 0:  # начать игру
                current_state = 1
                background_sound.play()
            if current_state == 1:  # идет игра, нажали PAUSE
                current_state = 2
            if current_state == 2:  # сняли с паузы
                current_state = 1
...
background_sound = pg.mixer.Sound("race-track.wav")
...
while True:
    ...
    for i in pg.event.get():
        quit(i)
        current_state = change_state(i, current_state, background_sound)
